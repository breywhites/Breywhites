...
-- ---------------------------------------------------------------------
-- Wrapped so that, if this project won't accept storage changes from SQL, the
-- rest of the setup still completes (then add the bucket + 3 policies in Storage).
do $$
begin
  ...
  if not exists (select 1 from pg_policies where schemaname = 'storage' and tablename = 'objects' and policyname = 'photos admin insert') then
    create policy "photos admin insert" on storage.objects for insert to authenticated
      with check (bucket_id = 'photos' and public.is_admin());
  end if;
  if not exists (select 1 from pg_policies where schemaname = 'storage' and tablename = 'objects' and policyname = 'photos admin update') then
    create policy "photos admin update" on storage.objects for update to authenticated
      using (bucket_id = 'photos' and public.is_admin());
  end if;
  if not exists (select 1 from pg_policies where schemaname = 'storage' and tablename = 'objects' and policyname = 'photos admin delete') then
    create policy "photos admin delete" on storage.objects for delete to authenticated
      using (bucket_id = 'photos' and public.is_admin());
  end if;
exception when others then
  raise notice 'Photo storage was not set up from SQL (%). In Supabase → Storage, create a public bucket named "photos" and allow admins to upload.', sqlerrm;
end $$;
